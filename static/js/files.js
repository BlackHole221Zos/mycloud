...
function openShareModal(fileId, fileName) {
    ...
    if (!modal) {
        alert('Модалка не найдена! Проверь HTML');
        return;
    }
...
function openGroupShareModal(groupId, fileId, fileName) {
    currentShareFileId = fileId;
    const modal = document.getElementById('shareModal');
    ...
    if (form) {
        // отдельный эндпоинт для группового файла
        form.action = `/groups/${groupId}/share/${fileId}`;
        form.reset();
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    shareForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // важно: чтобы не упасть, убеждаемся, что форма есть
            if (shareForm) {
...
window.openGroupShareModal = openGroupShareModal;
