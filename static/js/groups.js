...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Обработчик для приглашений (модалка с подтверждением)
    const genBtn = document.getElementById('gen-btn');
    const inviteConfirmModal = document.getElementById('inviteConfirmModal');
    const inviteConfirmYes = document.getElementById('invite-confirm-yes');

    if (genBtn && inviteConfirmModal && inviteConfirmYes) {
        let pendingGenerate = null;

        async function doGenerateLink(btn) {
            btn.disabled = true;
            ...
            if (expiresSelect) {
                fd.append('expires_in', expiresSelect.value);
            }
        ...
        genBtn.addEventListener('click', function (e) {
            e.preventDefault();
            pendingGenerate = this;
            inviteConfirmModal.classList.add('active');
        });

        inviteConfirmYes.addEventListener('click', function () {
            inviteConfirmModal.classList.remove('active');
            if (pendingGenerate) {
                doGenerateLink(pendingGenerate);
