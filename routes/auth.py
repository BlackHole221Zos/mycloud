...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            # === АВТО-СОХРАНЕНИЕ ФАЙЛА ПОСЛЕ ЛОГИНА ===
            token = session.pop('auto_save_token', None)
            if token:
                return redirect(url_for('files.auto_save_shared', token=token))
...
def register():
    if request.method == 'POST':
        ...
        # === АВТО-СОХРАНЕНИЕ ФАЙЛА ПОСЛЕ РЕГИСТРАЦИИ ===
        token = session.pop('auto_save_token', None)
        if token:
            return redirect(url_for('files.auto_save_shared', token=token))
